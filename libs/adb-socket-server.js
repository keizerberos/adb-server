...
class AdbSocketServer {
	...
	startServer(clients, clusters, devices) {
		...
		fastServer.get("/clusters", (req, res) => {
			res.setHeader('Content-Type', 'application/json');
			res.send(JSON.stringify(clusters.map(cluster => { return { uuid: cluster.uuid, devices: cluster.devices, network: cluster.address, config: cluster.config }; })));
		});
	...
	startClientCenter(clients, clusters, devices){
		...
		io.on("disconnect", (e) => {
			Log.i("Disconnected from" + process.env['ADB_CENTER']);
			Log.o(e);
		});
		io.on("connect", () => {
