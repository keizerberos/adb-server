...
class LoginModule {
	...
	when(ev, _data) {
		...
		if (ev == "io.connection") {
			...
			this.socket.on("auth.login", (data) => {
				...
				if (user != undefined) {
					...
					if (data.i != undefined && data.t != undefined) {
						...
						console.log("auth test t i");
						if (self.tokens[data.t] != undefined) {
							console.log("auth token mem");
							this.socket.emit("auth", { t: data.t, i: data.i, n: data.n });
							_data.configSocket();
						} else {
			...
			this.socket.on("user.load", (data) => {
				console.log("load users");
				const users = new UserEntity(self.dbm).all();
				this.socket.emit("user.load", users.map(u => { return {id:u.id, username:u.username, fullname:u.fullname, roles:u.roles, mail:u.mail, enabled:u.enabled}; }));
			});
			...
			this.socket.on("user.save", (data) => {
				...
				const user = new UserEntity(self.dbm);
				user.set(data);
				this.socket.emit("user.save", user.save());
			});
			this.socket.on("user.delete", (data) => {				
				...
				this.socket.emit("user.delete", user.delete());
			});
		}
	}
}
